...
def PROFILE(call, output=None, stats=['time', 'calls', 'cumulative', 'pcalls']):
    """Run the call(s) inside profiler
    @var call: either function or list of functions
    ...
    @keyword output: where to save the stats, if empty, it will be printed
          to stdout
    @keyword stats: list of statistical outputs,
...
def DEBUG_CYCLE(stmt, setup=None,
    ...
    @keyword setup: initialization (ie. "import sys")
    ...
    *after* execution of the main call. If debug_stopper
    ...
    @keyword onerror: string (like the setup) which will
    ...
    def x(obj, eng):
        ...
        while storage[2]:
            try:
                ...
                _setup = ';'.join(['%s=_it[0].topass[\'%s\']' % (k, k) for k, v in to_pass.items()])
                _setup += '\nobj=_it[0].obj\neng=_it[0].eng'
                _setup += '\n%s' % setup
                ...
                if '<timeit-src>' in lasterr[-2]:
                    sys.stderr.write('Error most likely in compilation, printing the source code:\n%s%s\n%s\n' %
                                     ('=' *60, t.src, '=' * 60))
                    break
...
def CALLFUNC(func, outkey=None, debug=False, stopper=None,
    ...
    @param func: identification of the function, it can be either
        ...
        itself
    @keyword outkey: results of the call will be stored inside
        eng.setVar(outkey) if outkey != None
    @keyword debug: boolean, if True, we will run the call in a
        loop, reloading the module after each error
    @keyword stopper: a callable which will receive obj, eng
        after each run. If the callable returns True, we will
        stop running the func (only applicable when debug=True)
    @keyword args: params passed on to the function
    @keyword ekeys: dictionary of key:value pairs, we will take
        'value' from the engine, and pass it to the function under
        the 'key' name.
    @keyword okeys: the same as ekeys, only that values are taken
        from the obj
    @keyword oeargs: definition of arguments that should be put
    ...
    @keyword **kwargs: other kwargs passed on to the function
            ...
            for key in oeargs:
                ...
                if first_key == 'O':
                    ...
                    if key in obj:
                        ...
                        raise Exception("%s is not inside obj nor eng, try specifying Okey or Ekey" % key)
        except Exception, msg:
            eng.log.error(traceback.format_exc())
            eng.log.error('Check your "oeargs" configuration. Key "%s" not available' % key)
            sys.exit(1)
...
def debug_simple(func, *args, **kwargs):
    """Runs func with *args, **kwargs and reloads it
